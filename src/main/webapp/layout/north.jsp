	...
	function logoutFun(b) {
		$.getJSON('${pageContext.request.contextPath}/userController/logout', {
			...
			if (b) {
				...
				location.href = '${pageContext.request.contextPath}/login.jsp?username=${sessionInfo.username}&schoolId=${sessionInfo.schoolId}';
			}
		});
	}
	...
	*/
	function editCurrentUser() {
		parent.$.modalDialog({
			...
			height : 330,
			iconCls : 'pencil',
			href : '${pageContext.request.contextPath}/userController/editCurrentUserPage',
			buttons : [ {
				text : '修改',
				handler : function() {
...
<div id="sessionInfoDiv" style="position: absolute; right: 0px; top: 0px;" class="alert alert-info">
	<c:if test="${sessionInfo.id != null}">[<strong><span style="color:red">${sessionInfo.name}</span></strong>]，欢迎你！您当前所在[<strong>${sessionInfo.schoolName}</strong>]校区！</c:if>
</div> 
<div style="position: absolute; right: 0px; top: 0px;" class="alert alert-info">
	<a href="javascript:void(0);" class="easyui-menubutton" data-options="menu:'#layout_north_pfMenu',iconCls:'color_wheel'">更换皮肤</a> <a href="javascript:void(0);" class="easyui-menubutton" data-options="menu:'#layout_north_kzmbMenu',iconCls:'cog'">控制面板</a> <a href="javascript:void(0);" class="easyui-menubutton" data-options="menu:'#layout_north_zxMenu',iconCls:'index_exit'">注销</a>
</div>
...
<div id="layout_north_kzmbMenu" style="width: 100px; display: none;">
	...
	<div onclick="editCurrentUser();">修改个人资料</div>
</div>
<div id="layout_north_zxMenu" style="width: 100px; display: none;">
	<div onclick="logoutFun();">锁屏</div>
	<div class="menu-sep"></div>
	<div onclick="logoutFun();">重新登录</div>
	<div onclick="logoutFun(true);">退出</div>
</div>